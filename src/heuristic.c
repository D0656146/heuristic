...
void* HillClimbing(void* problem_dataset,
                   void* init_solution,
  ...
  for (int c=0; c<iterations; c++) {
    ...
    void** neighbor_solutions = FindNeighbors(problem_dataset,
                                current_solution,
                                &neighbor_quantity);
...
void* RandomRestartHillClimbing(void* problem_dataset,
  ...
  for (int c=0; c<restart_times; c++) {
    ...
    void* local_optimal = HillClimbing(problem_dataset,
                                       random_init_solution,
                                       Estimate,
                                       FindNeighbors,
                                       iterations,
                                       local_loggings);
    if (Estimate(problem_dataset, best_solution) < Estimate(problem_dataset, local_optimal)) {
      free(best_solution);
